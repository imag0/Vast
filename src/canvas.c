...
static void handle_motion(AInputEvent*e){
    ...
    if(masked==AMOTION_ACTION_DOWN){
        ...
        if(G.zenMode||G.tool==MODE_ERASE){G.fingerCount=1;G.fingerPanning=G.zenMode?0:1;G.fingerMoved=0;G.fingerDownX=x;G.fingerDownY=y;G.lastFingerX=x;G.lastFingerY=y;G.fingerDownRef=(ObjRef){SEL_NONE,-1};G.selectionDragging=0;cancel_pen_hold_timer();render_event(now,1);return;}
