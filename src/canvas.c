...
typedef enum { ACT_DRAW=1, ACT_ERASE=2, ACT_CLEAR=3 } ActionType;
typedef struct { ActionType type; int* ids; int n,cap; } Action;
typedef struct { float ax,ay,bx,by; int active; } Measure;
typedef enum { MODE_PEN=0, MODE_ERASE=1, MODE_MEASURE=2, MODE_HIGHLIGHTER=3, MODE_SELECT=4, MODE_SHAPE=5 } ToolMode;
...
typedef struct {
    ...
    int currentStroke; Action eraseAction; int erasing;
    ...
    int stylusDown,stylusPointerId,stylusToolType; int fingerCount,fingerPanning,fingerMoved,fingerId0,fingerId1; float lastFingerX,lastFingerY,lastMidX,lastMidY,lastPinchDist,fingerDownX,fingerDownY; ObjRef fingerDownRef;
    ...
    ImageObj images[32],imageCancelStart; int imageN,imagesDirty,imageContentRevision; char imagePath[512]; int photoMode,selectedImage,imageDragging,imageGesture,imageCancelValid,photoAngleSnap; float imageStartX,imageStartY,imageStartW,imageStartH,imageStartRot,imageStartCX,imageStartCY,imageGestureDist,imageGestureAngle,imageGestureMidX,imageGestureMidY;
...
static void free_action(Action*a){if(a->ids)free(a->ids);memset(a,0,sizeof(*a));}
...
static int action_hits_locked_layer(const Action*a){if(!a)return 0;for(int i=0;i<a->n;i++){int id=a->ids[i];if(id>=0&&id<G.strokeN&&stroke_layer_locked(&G.strokes[id]))return 1;}return 0;}
static void undo_action(void){G.clearArmed=0;if(G.actionCursor<=0){set_toast(2,0);return;}Action*a=&G.actions[G.actionCursor-1];if(action_hits_locked_layer(a)){set_toast(9,0);return;}G.actionCursor--;if(a->type==ACT_DRAW){for(int i=0;i<a->n;i++)if(a->ids[i]>=0&&a->ids[i]<G.strokeN)G.strokes[a->ids[i]].active=0;ocr_set_action_active(a,0);}else{for(int i=0;i<a->n;i++)if(a->ids[i]>=0&&a->ids[i]<G.strokeN)G.strokes[a->ids[i]].active=1;ocr_set_action_active(a,1);}G.saveDirty=1;G.minimapDirty=1;set_toast(1,0);}
static void redo_action(void){G.clearArmed=0;if(G.actionCursor>=G.actionN)return;Action*a=&G.actions[G.actionCursor];if(action_hits_locked_layer(a)){set_toast(9,0);return;}G.actionCursor++;if(a->type==ACT_DRAW){for(int i=0;i<a->n;i++)if(a->ids[i]>=0&&a->ids[i]<G.strokeN)G.strokes[a->ids[i]].active=1;ocr_set_action_active(a,1);}else{for(int i=0;i<a->n;i++)if(a->ids[i]>=0&&a->ids[i]<G.strokeN)G.strokes[a->ids[i]].active=0;ocr_set_action_active(a,0);}G.saveDirty=1;G.minimapDirty=1;}
...
static int save_images(void){
    if(!G.imagesDirty)return 1;if(!G.imagePath[0])return 0;char tmp[544];void*f=atomic_write_begin(G.imagePath,tmp,sizeof(tmp));if(!f)return 0;uint32_t c=0;for(int i=0;i<G.imageN;i++)if(G.images[i].active&&G.images[i].px)c++;
...
static uint32_t* load_media_full(int64_t id,int*outW,int*outH){
    *outW=*outH=0;if(!G.activity||!G.activity->env)return 0;JNIEnv*e=G.activity->env;jclass cls=(*e)->FindClass(e,"com/ayomi/infinitecanvas/VastUi");if(!cls||jni_clear(e))return 0;
    jmethodID infoMethod=(*e)->GetStaticMethodID(e,cls,"photoInfo","(Landroid/app/Activity;JI)[I"),tileMethod=(*e)->GetStaticMethodID(e,cls,"photoTile","(Landroid/app/Activity;JIIIII)[I");if(!infoMethod||!tileMethod||jni_clear(e)){(*e)->DeleteLocalRef(e,cls);return 0;}
    jintArray info=(jintArray)(*e)->CallStaticObjectMethod(e,cls,infoMethod,G.activity->clazz,(jlong)id,(jint)8192);if(jni_clear(e)||!info||(*e)->GetArrayLength(e,info)<6){if(info)(*e)->DeleteLocalRef(e,info);(*e)->DeleteLocalRef(e,cls);return 0;}jint v[6];(*e)->GetIntArrayRegion(e,info,0,6,v);(*e)->DeleteLocalRef(e,info);if(jni_clear(e)){(*e)->DeleteLocalRef(e,cls);return 0;}
    int sourceW=v[0],sourceH=v[1],sample=v[2],orientation=v[3],w=v[4],h=v[5];uint64_t count=(uint64_t)(uint32_t)w*(uint64_t)(uint32_t)h;if(sourceW<=0||sourceH<=0||sample<=0||w<=0||h<=0||w>8192||h>8192||count>67108864u){(*e)->DeleteLocalRef(e,cls);return 0;}
    uint32_t*out=(uint32_t*)malloc((size_t)count*sizeof(uint32_t));if(!out){(*e)->DeleteLocalRef(e,cls);return 0;}const int outputTile=1024;int ok=1;
    for(int top=0;ok&&top<sourceH;top+=outputTile*sample)for(int left=0;ok&&left<sourceW;left+=outputTile*sample){int right=mini(sourceW,left+outputTile*sample),bottom=mini(sourceH,top+outputTile*sample);jintArray tile=(jintArray)(*e)->CallStaticObjectMethod(e,cls,tileMethod,G.activity->clazz,(jlong)id,(jint)left,(jint)top,(jint)right,(jint)bottom,(jint)sample);if(jni_clear(e)||!tile||(*e)->GetArrayLength(e,tile)<2){if(tile)(*e)->DeleteLocalRef(e,tile);ok=0;break;}jint wh[2];(*e)->GetIntArrayRegion(e,tile,0,2,wh);int tw=wh[0],th=wh[1];jsize len=(*e)->GetArrayLength(e,tile);if(tw<=0||th<=0||(uint64_t)tw*(uint64_t)th>(uint64_t)(len-2)){(*e)->DeleteLocalRef(e,tile);ok=0;break;}jint*pixels=(jint*)malloc((size_t)tw*th*sizeof(jint));if(!pixels){(*e)->DeleteLocalRef(e,tile);ok=0;break;}(*e)->GetIntArrayRegion(e,tile,2,tw*th,pixels);(*e)->DeleteLocalRef(e,tile);if(jni_clear(e)){free(pixels);ok=0;break;}int baseX=left/sample,baseY=top/sample;for(int yy=0;yy<th;yy++)for(int xx=0;xx<tw;xx++){int gx=baseX+xx,gy=baseY+yy,ox=gx,oy=gy;if(orientation==90){ox=w-1-gy;oy=gx;}else if(orientation==180){ox=w-1-gx;oy=h-1-gy;}else if(orientation==270){ox=gy;oy=h-1-gx;}if(ox>=0&&ox<w&&oy>=0&&oy<h)out[(size_t)oy*w+ox]=(uint32_t)pixels[yy*tw+xx]&0x00ffffffu;}free(pixels);}
    (*e)->DeleteLocalRef(e,cls);if(!ok){free(out);return 0;}*outW=w;*outH=h;return out;
}
...
static void import_gallery_item(int idx){G.imageContentRevision++;if(idx<0||idx>=G.thumbN||G.imageN>=32)return;int w=0,h=0;uint32_t*px=load_media_full(G.thumbs[idx].mediaId,&w,&h);if(!px||w<=0||h<=0){if(px)free(px);set_toast(5,0);return;}ImageObj*im=&G.images[G.imageN++];memset(im,0,sizeof(*im));im->px=px;im->pw=w;im->ph=h;im->active=1;float ww=800.0f,hh=ww*(float)h/(float)w;if(hh>800.0f){hh=800.0f;ww=hh*(float)w/(float)h;}float cx=(G.screenW*0.5f-G.offX)/G.scale,cy=(G.screenH*0.5f-G.offY)/G.scale;im->w=ww;im->h=hh;im->x=cx-ww*.5f;im->y=cy-hh*.5f;G.imagesDirty=1;G.minimapDirty=1;G.selectedImage=G.imageN-1;G.photoMode=1;save_images();G.galleryOpen=0;free_thumbs();set_toast(3,0);}
...
static void project_path_for(int idx,int kind,char*out,int cap){if(!out||cap<=0){return;}out[0]=0;if(!G.activity||!G.activity->internalDataPath)return;idx=maxi(0,mini(3,idx));const char*b=G.activity->internalDataPath;const char*base[5]={"canvas","canvas","images","frames","workspace"};const char*ext[5]={".icv",".meta",".icv",".vfr",".v3"};kind=maxi(0,mini(4,kind));if(idx==0){if(kind==0)snprintf(out,(size_t)cap,"%s/canvas.icv",b);else if(kind==1)snprintf(out,(size_t)cap,"%s/canvas.meta",b);else if(kind==2)snprintf(out,(size_t)cap,"%s/images.icv",b);else if(kind==3)snprintf(out,(size_t)cap,"%s/frames.vfr",b);else snprintf(out,(size_t)cap,"%s/workspace.v3",b);}else snprintf(out,(size_t)cap,"%s/%s_%d%s",b,base[kind],idx,ext[kind]);}
...
static int project_recover_incomplete_duplicate(int idx){
    ...
    for(int k=0;k<5;k++){char out[512],stage[544];project_path_for(idx,k,out,sizeof(out));snprintf(stage,sizeof(stage),"%s.dup",out);if(!remove_file_if_present(stage))ok=0;if(!remove_file_if_present(out))ok=0;}
...
static int project_slot_has_data(int idx){if(!project_recover_incomplete_duplicate(idx))return 1;char p[512];for(int k=0;k<5;k++){project_path_for(idx,k,p,512);if(file_exists_local(p))return 1;}return 0;}
...
static void clear_document_memory(void){
    erase_index_reset();
    for(int i=0;i<G.imageN;i++)if(G.images[i].px){free(G.images[i].px);G.images[i].px=0;}memset(G.images,0,sizeof(G.images));G.imageN=0;G.imagesDirty=0;
    ...
    for(int i=0;i<G.actionN;i++)free_action(&G.actions[i]);if(G.actions){free(G.actions);G.actions=0;}G.actionN=G.actionCap=G.actionCursor=0;free_action(&G.eraseAction);G.currentStroke=-1;G.erasing=0;
...
static int duplicate_current_project(void){
    ...
    char src[5][512],out[5][512],stage[5][544],marker[544],markerTmp[576];memset(stage,0,sizeof(stage));int present[5]={0,0,0,0,0},copied=0,marked=0;project_duplicate_marker_path(dst,marker,sizeof(marker));
    for(int k=0;k<5;k++){project_path_for(G.projectIndex,k,src[k],512);project_path_for(dst,k,out[k],512);int n=snprintf(stage[k],sizeof(stage[k]),"%s.dup",out[k]);if(n<=0||n>=(int)sizeof(stage[k]))goto fail;remove(stage[k]);if(file_exists_local(src[k])){present[k]=1;if(!copy_file_local(src[k],stage[k]))goto fail;copied=1;}}
    if(!copied)goto fail;
    ...
    for(int k=0;k<5;k++)if(present[k]&&rename(stage[k],out[k])!=0)goto fail;
...
fail:
    {int clean=1;for(int k=0;k<5;k++){if(stage[k][0]&&!remove_file_if_present(stage[k]))clean=0;if(marked&&out[k][0]&&!remove_file_if_present(out[k]))clean=0;}if(!fsync_parent_directory(marker))clean=0;if(marked&&clean){if(!remove_file_if_present(marker))clean=0;else if(!fsync_parent_directory(marker))clean=0;}(void)clean;}return -2;
}
...
static int dock_active(int i){return (i==0&&G.tool==MODE_PEN)||(i==1&&G.tool==MODE_ERASE)||(i==2&&G.tool==MODE_MEASURE)||(i==7&&G.pressurePanel)||(i==8&&G.settingsPanel)||(i==3&&G.galleryOpen);}
...
static void draw_strokes(Surf*s){
    ...
    int eraseView=G.erasing||G.tool==MODE_ERASE;
...
static const char*tool_name(void){if(G.photoMode)return "Photo";if(G.tool==MODE_PEN)return "Pen";if(G.tool==MODE_HIGHLIGHTER)return "Marker";if(G.tool==MODE_ERASE)return "Eraser";if(G.tool==MODE_MEASURE)return "CAD";if(G.tool==MODE_SELECT)return "Select";if(G.tool==MODE_SHAPE)return "Shape";return "Tool";}
...
static void nui_sync(void){
    ...
    if(stateChanged||themeChanged){
        ...
        for(int i=0;i<8;i++)nui_button_style(e,NU.rail[i],rail_active(i),0);
        int tog=!G.presentationMode&&!G.settingsPanel&&!G.galleryOpen&&!G.calibrationOpen&&!G.projectsPanel&&!G.editorOpen;nui_vis(e,NU.inkToggle,tog);int er=G.tool==MODE_ERASE;nui_text(e,NU.inkToggle,er?"E":"P");nui_bg(e,NU.inkToggle,C_GLASS2,245,er?C_DANGER:C_ACCENT,210,maxi(1,us(2)),(float)us(28),1);nui_text_color(e,NU.inkToggle,er?C_DANGER:C_ACCENT,255);
...
static void draw_ink_toggle(Surf*s){if(G.presentationMode||G.settingsPanel||G.galleryOpen||G.calibrationOpen||G.projectsPanel||G.editorOpen)return;float r=(float)us(88),cx=0.0f,cy=(float)s->h,ix=r*.43f,iy=cy-r*.43f;uint32_t ac=G.tool==MODE_ERASE?mix_color(C_TEXT,C_DANGER,22):C_TEXT;alpha_circle(s,cx,cy,r+us(4),C_BLACK,32);aa_circle(s,cx,cy,r+us(2),C_TEXT,65);aa_circle(s,cx,cy,r,mix_color(C_GLASS2,C_TEXT,7),233);line_icon(s,G.tool==MODE_ERASE?1:0,ix,iy,1.30f,ac);}
...
static void render(void){if(INPUT_BATCH_ACTIVE){INPUT_FRAME_PENDING=1;return;}if(!G.window)return;if(!G.stylusDown&&G.tool==MODE_ERASE)erase_index_prepare();int done=0;double frameStart=perf_us();
#ifdef VAST_GPU
...
// ---------- UI interaction ----------
enum {
    UI_NONE=0,UI_PEN,UI_HIGHLIGHT,UI_ERASE,UI_MEASURE,UI_IMPORT,UI_PHOTO_MODE,UI_FRAMES,UI_UNDO,UI_REDO,UI_FIT,UI_PRESSURE,UI_MINIMAP,UI_SETTINGS,UI_CLEAR,UI_RAIL_OPEN,
    ...
    UI_PROJECT_CLOSE,UI_PROJECT_RENAME,UI_PROJECT_DUP,UI_PROJECT0,UI_PROJECT1,UI_PROJECT2,UI_PROJECT3,
...
static int ui_hit_projects(float x,float y){int m=margin_ui(),w=us(720),h=us(540),px=(G.screenW-w)/2,py=(G.screenH-h)/2;if(hit_box(x,y,px+w-us(132),py+us(20),px+w-us(24),py+us(64)))return UI_PROJECT_CLOSE;for(int i=0;i<4;i++){int col=i%2,row=i/2,bx=px+us(28)+col*us(330),by=py+us(116)+row*us(132);if(hit_box(x,y,bx,by,bx+us(304),by+us(102)))return UI_PROJECT0+i;}if(hit_box(x,y,px+us(28),py+h-us(94),px+us(220),py+h-us(38)))return UI_PROJECT_RENAME;if(hit_box(x,y,px+us(234),py+h-us(94),px+us(438),py+h-us(38)))return UI_PROJECT_DUP;return UI_NONE;}
...
static int anim_timer_cb(int fd,int events,void*data){
    ...
    int scanning=G.ocrInitialPending;if(idle){if(G.workspaceSavePending&&save_workspace())G.workspaceSavePending=0;perf_report();perf_dump();scanning=ocr_initial_scan_step();ocr_pending_flush();if(G.inkSavePending){if(save_canvas()){G.inkSavePending=0;ocr_checkpoint();}else set_toast(8,now);}}
    ...
    if(cameraChanged||spatialChanged||textChanged||uiChanged)render();
    if(settled){if(scanning||G.inkSavePending||G.workspaceSavePending||G.ocrPending||PERF_DIRTY)arm_anim_timer_ms(64,0);else cancel_anim_timer();}
    return 1;
}
...
static void reveal_canvas_chrome(void){G.zenMode=0;G.railOpen=1;start_anim_timer();arm_rail_timer();}
static float pen_hold_buffer_px(void){float inv=1.0f/sqrtf(fmax2(G.scale,0.22f));return clampf(8.0f*inv,8.0f,20.0f);}
static void delete_selected_image(void){G.imageContentRevision++;int id=G.selectedImage;if(id<0||id>=G.imageN||G.lockPhotos)return;if(G.images[id].px)free(G.images[id].px);for(int i=id+1;i<G.imageN;i++)G.images[i-1]=G.images[i];G.imageN--;if(G.imageN>=0)memset(&G.images[G.imageN],0,sizeof(ImageObj));G.selectedImage=-1;G.imagesDirty=1;G.minimapDirty=1;}
...
static void handle_ui(int id,int64_t ms){
    ...
    switch(id){
        ...
        case UI_PROJECT_CLOSE:G.projectsPanel=0;break;case UI_PROJECT_RENAME:editor_open(4,G.projectIndex,G.projectNames[G.projectIndex]);break;case UI_PROJECT_DUP:{int q=duplicate_current_project();if(q>=0)set_toast(11,ms);else set_toast(q==-1?10:8,ms);}break;
        ...
        case UI_INK_TOGGLE:G.photoMode=0;G.pressurePanel=0;G.tool=G.tool==MODE_ERASE?MODE_PEN:MODE_ERASE;break;
...
static void cancel_canvas_contact(void){
    fling_cancel();
    if(G.stylusDown){
        if(G.erasing){for(int i=0;i<G.eraseAction.n;i++){int id=G.eraseAction.ids[i];if(id>=0&&id<G.strokeN)G.strokes[id].active=1;}free_action(&G.eraseAction);G.saveDirty=1;}
    ...
    G.stylusDown=G.erasing=G.hoverActive=G.stylusToolType=0;G.stylusPointerId=-1;
...
static void handle_motion(AInputEvent*e){
    ...
    if(hasStylus){
        ...
        int tt=AMotionEvent_getToolType(e,(size_t)stylusIndex),tempErase=G.tool==MODE_ERASE||tt==TOOL_ERASER||G.buttonQuickErase;
        if(masked==AMOTION_ACTION_DOWN){
            ...
            else if(G.tool==MODE_MEASURE&&!tempErase)start_measure(sx,sy);else if(tempErase){G.erasing=1;memset(&G.eraseAction,0,sizeof(G.eraseAction));G.eraseAction.type=ACT_ERASE;erase_at(sx,sy);}else start_stroke(sx,sy,p);
            ...
            else{size_t hist=AMotionEvent_getHistorySize(e);for(size_t j=0;j<hist;j++){G.sampleTimeNs=AMotionEvent_getHistoricalEventTime(e,j);float hx=AMotionEvent_getHistoricalX(e,(size_t)stylusIndex,j),hy=AMotionEvent_getHistoricalY(e,(size_t)stylusIndex,j),hp=AMotionEvent_getHistoricalPressure(e,(size_t)stylusIndex,j);if(G.measuring)update_measure(hx,hy);else if(G.erasing)erase_at(hx,hy);else add_stroke_point(hx,hy,hp);}G.sampleTimeNs=AMotionEvent_getEventTime(e);if(G.measuring)update_measure(sx,sy);else if(G.erasing)erase_at(sx,sy);else add_stroke_point(sx,sy,p);}
            render_event(now,0);
        }else if((masked==AMOTION_ACTION_UP||masked==AMOTION_ACTION_CANCEL)&&G.stylusDown){
            int cancelled=masked==AMOTION_ACTION_CANCEL,wasErasing=G.erasing;
            if(wasErasing){if(!cancelled)erase_at(sx,sy);G.stylusDown=0;G.hoverActive=0;render_event(now,1);}
            ...
            else if(G.erasing){if(cancelled){for(int i=0;i<G.eraseAction.n;i++)if(G.eraseAction.ids[i]>=0&&G.eraseAction.ids[i]<G.strokeN)G.strokes[G.eraseAction.ids[i]].active=1;free_action(&G.eraseAction);}else if(G.eraseAction.n){ocr_set_action_active(&G.eraseAction,0);push_action(&G.eraseAction);}else free_action(&G.eraseAction);G.erasing=0;}
            ...
            else if(wasErasing)schedule_ink_save();
    ...
    if(masked==AMOTION_ACTION_DOWN){
        ...
        ObjRef pressedObject=object_hit(x,y);if(pressedObject.type==SEL_NONE)reveal_canvas_chrome();
        ...
        if(G.zenMode||G.tool==MODE_ERASE){G.fingerCount=1;G.fingerPanning=1;G.fingerMoved=0;G.fingerDownX=x;G.fingerDownY=y;G.lastFingerX=x;G.lastFingerY=y;G.fingerDownRef=(ObjRef){SEL_NONE,-1};G.selectionDragging=0;cancel_pen_hold_timer();render_event(now,1);return;}
        ...
        cancel_pen_hold_timer();ocr_move_commit();G.snapGuideX=G.snapGuideY=0;
        ...
        if(G.tool!=MODE_ERASE&&G.photoMode&&photo_begin_gesture(x0,y0,x1,y1)){G.fingerCount=2;G.twoTapCandidate=0;return;}
        if(G.tool!=MODE_ERASE&&G.selectedRefN>0&&selection_gesture_contains(x0,y0,x1,y1)){if(G.transformSnapshotN<=0)selection_snapshot_begin();G.selectionDragging=0;G.selectionGesture=1;G.selectionGestureDist=fmax2(4.0f,sqrtf(dx*dx+dy*dy));G.selectionGestureAngle=atan2f(dy,dx);G.selectionGestureMidWX=(mx-G.offX)/G.scale;G.selectionGestureMidWY=(my-G.offY)/G.scale;G.fingerCount=2;G.twoTapCandidate=0;return;}
...
static void cb_window_destroyed(ANativeActivity*a,ANativeWindow*w){(void)a;(void)w;cancel_anim_timer();save_all_document();save_project_prefs();nui_destroy();aui_void("destroy");G.systemImeActive=0;AUI_PANEL_HASH=AUI_CHROME_HASH=0;
#ifdef VAST_GPU
    gpu_destroy();
#endif
    G.window=0;}
